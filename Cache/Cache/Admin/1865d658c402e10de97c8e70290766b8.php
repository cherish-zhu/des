  ...
  <tr>
    <td height="42" colspan="2"><div style="width:200px; height:42px; float:left">联系地址：</div>
      ...
      <select name="city" id="city">
        <option value="0" >-</option>
        <?php
 foreach($pro as $key => $val){ $sel = ''; if($val['cityId'] == $u['city']) $sel = 'selected = "selected"'; echo '<option value="'.$val['cityId'].'" '.$sel.' >'.$val['cityName'].'</option>'; } ?>
      </select>
      <select name="county" id="county">
        <?php
 foreach($cit as $k => $v){ $sel = ''; if($v['districtId'] == $u['county']) $sel = 'selected = "selected"'; echo '<option value="'.$v['districtId'].'" '.$sel.' >'.$v['districtName'].'</option>'; } ?>
      </select>
