					...
					<table class="table table-striped">
                        ...
                        <?php foreach($menu as $key => $k){?>
							<tr>
								...
								<td  width="150" style="float:right"><a href="/admin/menu/edit?id=<?php echo $k['id'] ?>">编辑</a>&nbsp;&nbsp;<a href="/admin/menu/menuStatus/status/<?php echo $k['status'] ?>?id=<?php echo $k['id'] ?>" class="menuStatus" id="<?php echo $k['status'] ?>-<?php echo $k['id'] ?>"><?php if($k["status"] == 1): ?>禁用<?php else: ?>启用<?php endif; ?></a>&nbsp;&nbsp;<a href="/admin/menu/addFrom?id=<?php echo $k['id'] ?>">添加子菜单</a></td>
							</tr>
							<?php }?>                     
						</tbody>
					</table>
